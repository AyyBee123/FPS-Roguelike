...
func create_aura(projectile: Variant, damage: float):
	if projectile.get_node_or_null("Circle of Light Aura"):
		...
		if projectile.is_in_group("Ability"):
			_aura.scale = Vector3.ONE * get_stat_value("Range") / projectile.scale
		else:
			_aura.scale = Vector3.ONE * get_stat_value("Range", player.weapons_manager.current_arm.splash_radius) / projectile.scale
		return
	var aura = AURA.instantiate()
	aura.visible = false
	aura.damage = get_stat_value("Damage", damage)
	if projectile.is_in_group("Ability"):
		aura.scale = Vector3.ONE * get_stat_value("Range") / projectile.scale
	else:
		aura.scale = Vector3.ONE * get_stat_value("Range", player.weapons_manager.current_arm.splash_radius) / projectile.scale
	aura.player = player
