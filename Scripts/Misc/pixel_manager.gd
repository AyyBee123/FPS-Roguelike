...
# Nodes To Pixelise
@export var pixel_node_paths : Array[NodePath]
...
func _ready():
	...
	for pixel_node_path in pixel_node_paths:
		...
		pixel_node.get_parent().add_child.call_deferred(pixel_sprite)
		pixel_sprite.transform.origin = pixel_node.transform.origin + sprite_bounds.get_center()
		
		pixel_node.get_parent().remove_child.call_deferred(pixel_node)
		pixel_view.add_child.call_deferred(pixel_node)
...
func _process(_delta):
	for i in range(pixel_nodes.size()):
		...
		var sprite_pos: Vector3 = pixel_nodes[i].transform.origin + sprite_boundses[i].get_center()
		var main_cam: Transform3D = get_viewport().get_camera_3d().global_transform
